...
This document describes the target split configuration model. The current implementation still reads `packages.toml` until the migration is implemented.
...
class RawAurSource(BaseModel):
    ...
    package: str
    architectures: dict[Arch, str] = Field(default_factory=dict)
...
def normalize_aur_source(
    ...
    return AurArchSource(
        package=raw.package,
        upstream_architecture=required_arch_value(
            raw.architectures,
            arch,
            "AUR architecture mapping",
        ),
    )
